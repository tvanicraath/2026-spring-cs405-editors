...
*CS405 (Apekshaa's contribution)*

## Background and History

The Rabin-Karp algorithm was invented in 1987 by Michael O. Rabin and Richard M. Karp: both experts of theoretical computer science, a domain we have been diving into, in both CS304 and CS405. Both Rabin and Karp received the Turing Award for their individual exceptional contributions to the field of computer science.
...
Given:
- A text T = T[0], T[1],…,T[n−1] over an alphabet Σ, with ∣T∣=n
- A pattern P = P[0], P[1],…,P[m−1] over the same alphabet Σ, with ∣P∣=m
- Where m≤n

Find:

S = { i∈{0,1,…,n−m} ∣ T[i+j] = P[j] for all j ∈ {0,1,…,m−1} }
...
Time complexity: O(n * m) {COSTLY!! $$$}

For large inputs, far too slow.
...
```
new_hash = (old_hash - c_old × bᵐ⁻¹) × b + c_new
```
...
Pranav's editorial covers the KMP algorithm, which preprocesses the pattern into a failure function to avoid redundant comparisons. Here is how the two compare across scenarios.

| Scenario | Rabin-Karp | KMP |
|---|---|---|
| Single pattern, large text | O(n + m) avg | O(n + m) guaranteed |
| Worst-case guarantee | O(n * m) (pathological input) | O(n + m) always |
| Multiple patterns | Hash all patterns into a set; single pass over text | Must run separately per pattern (or use Aho-Corasick) |
...
A(x) = Σ {i} T[i] * x^i,  B(x) = Σ {j} P[m-1-j] * x^j

The product C = A * B gives at position i:

C[i + m - 1] = Σ {j=0}^{m-1} T[i+j] * P[j]

This is a dot product — equal to Σ P[j]^2 only when T[i+j] = P[j] for all j (for binary/character matching with the right formulation).

FFT computes this product in O(n log n) — better than Rabin-Karp's linear time? Not quite, but the constant matters less here; the FFT approach shines for:

### String Matching with Wildcards

Rabin-Karp (and KMP) struggle when the pattern contains wildcard characters ? that matches anything. FFT handles this elegantly.

Encode: let P[j] = 0 for wildcards. Define:

f(i) = Σ {j=0}^{m-1} (T[i+j] - P[j])^2 * [P[j] ≠ wildcard]
...
Rabin-Karp generalizes to 2D pattern matching — finding an m × m pattern inside an n × n grid — by hashing each row with a rolling hash, then hashing the resulting column of row-hashes with another rolling hash. This gives O(n^2) 2D pattern matching, which no failure-function-based algorithm achieves as cleanly. A fun exercise!
